...
speaking_topic_to_topic_info_map = {
    'speaking__listen_and_answer': {'display_name': 'Listen to the following question', 'emoji': '💭'},
    'speaking__express_an_opinion': {'display_name': 'Speak your opinion', 'emoji': '🗣️'},
    'speaking__debate': {'display_name': 'Discussion', 'emoji': '👩‍'},
    'speaking__describe_img': {'display_name': 'Describe a Picture', 'emoji': '🏞️'},
    'speaking__describe_charts': {'display_name': 'Give a speech about a grpaph given', 'emoji': '📊'},
}

writing_topic_to_topic_info_map = {
    'writing__dictation': {'display_name': 'Writing down exam', 'emoji': '✏️'},
    'writing__responding_to_an_email': {'display_name': 'Reply to an email', 'emoji': '✉️'},
    'writing__summarization': {'display_name': 'summarizing ', 'emoji': '✍️'},
...
if st.session_state["curr_page"] == "home":
    with con:
        st.title("Speaking & Writing Exam")


        tab1, tab2 = st.tabs(["Speaking Exam", "Writing Exam"])
        ...
        if st.session_state.exam_context.get("user_answer"):
            ...
            def get_speaking__listen_and_answer_result(answer_text):
                ...
                class Score(BaseModel):
                    reason: str = Field(description="Question에 대해 Your Answer가 적절한지에 대해 추론하라. in English.")
                    score: int = Field(description="Question에 대해 Your Answer가 적절한지에 대해 0~10점 사이의 점수를 부여하라. in English")
                parser = JsonOutputParser(pydantic_object=Score)
                format_instruction = parser.get_format_instructions()

                human_msg_prompt_template = HumanMessagePromptTemplate.from_template(
                    "{input}\n---\nQuestion에 대해 Your Answer가 적절한지에 대해 추론해서 0~10점 사이의 점수를 부여해라. 다음의 포맷에 맞춰 응답해라. in English.  : {format_instruction}",
...
# 
        ...
        if st.session_state.exam_context.get("user_answer"):
            ...
            with st.container(border=True):
                def get_speaking__express_opinion_result(answer_text):
                    model = ChatOpenAI(model="gpt-4-1106-preview")
                    class Score(BaseModel):
                        reason: str = Field(description="Question에 대한 의견을 힌국어로 대답하는 시험이다. Your Answer를 한국어로 번역해서 제시한 후, 의견을 적절히 구조적으로 응답했는지 추론하라. in English.")
                        score: int = Field(description="Question에 대해 Your Answer가 충분히 논리적으로 의견을 표현했는지에 대해 30자 이내로 작성하고, 0~10점 사이의 점수를 부여하라. 추가적으로 문법이나 발음에서 보완해야할 부분을 1가지씩 제안하라. in English.")
                    parser = JsonOutputParser(pydantic_object=Score)
                    format_instruction = parser.get_format_instructions()

                    human_msg_prompt_template = HumanMessagePromptTemplate.from_template(
                        "{input}\n---\nYour Answer는 한국어이다.Question에 대해 Your Answer가 충분히 논리적으로 의견을 표현했는지에 대해 0~10점 사이의 점수를 부여하라. 다음의 포맷에 맞춰 응답해라. in Englsh.  : {format_instruction}",
                        partial_variables={"format_instruction": format_instruction})
    ...
    if "messages" not in st.session_state.exam_context:
        system_prompt = """\
- you are an AI proctor .
- To improve users' Korean skills, discuss and ask questions about any topic."""
    ...
    with con1:
        ...
        if turn_len >= max_turn_len:
            ...
            def get_speaking__debate_result(conversation):
                model = ChatOpenAI(model="gpt-4-1106-preview")
                class Score(BaseModel):
                    reason: str = Field(description="주어진 대화에 대해 User가 얼마나 논리적이고 유창하게 영어로 응답하였는지 추론하라. in English.")
                    score: int = Field(description="주어진 대화에서 User의 응답에 대해 유창성과 논리성을 고려하여 0~10점 사이의 점수를 부여하라. user의 응답에서 부족한 점이 있다면, 발음과 문법 두가지를 markdown 형식으로 보완점을 알려줘라. In english.")
                parser = JsonOutputParser(pydantic_object=Score)
                format_instruction = parser.get_format_instructions()

                human_msg_prompt_template = HumanMessagePromptTemplate.from_template(
                    "{input}\n---\n주어진 대화에서 User의 응답에 대해 유창성과 논리성을 고려하여 0~10점 사이의 점수를 부여하라. 다음의 포맷에 맞춰 응답해라. in English.  : {format_instruction}",
                    partial_variables={"format_instruction": format_instruction})
